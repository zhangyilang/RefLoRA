...
from reflora import Refactorer, RefTrainer, PEFTArguments
...
class DataTrainingArguments:
    ...
    deterministic: Optional[bool] = field(
        default=True, metadata={"help": "Whether to use deterministic algorithms."}
    )
...
def main():
    ...
    set_seed(training_args.seed, data_args.deterministic)
    ...
    if not hasattr(config, 'cls_dropout'):
    ...
    if label_to_id is not None:
        ...
        model.config.label2id = {lable: id for id, lable in enumerate(label_list)}
        model.config.id2label = {id: label for label, id in config.label2id.items()}
    ...
    # PEFT config
    if ',' in peft_args.target_modules:
    ...
    # Need to reinitialize when `lora_B` is 0 and `reflora_warmup` is 0 (otherwise rank-deficient)
    if peft_args.refactor:
        refactorer = Refactorer(model, 
                                use_scalar=peft_args.use_scalar, 
                                warmup_steps=peft_args.reflora_warmup,
                                interval=peft_args.reflora_interval,
                                re_init=peft_args.init_lora_weights is True and peft_args.reflora_warmup  == 0)
    else:
        refactorer = None
    trainer = RefTrainer(
        model=model,
